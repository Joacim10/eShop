...
<script>
import LatestBlogCard from '@/components/Carousel/LatestBlog/LatestBlogCard.vue'
...
export default {
  ...
  methods: {
    ...
    changeHeightOnWindowResize() {
        // "REF" GÖR DET MÖJLIGT ATT KOMMA ÅT WIDTH I PIXLAR FÖR ATT RÄKNA UT HÖJDEN PÅ KORTEN
        this.cardHeight = this.$refs.blogCarousel.clientWidth * this.cardWidth * 0.01 * 0.7
    }
  },
  mounted: function() {
    ...
    //console.log('items.slice(0, reduceIndicators)', this.items.slice(0, this.reduceIndicators))
    this.$nextTick(() => {
  ...
  computed: {
    ...
    numberOfIndicators() {
      ...
      if (this.items.length) {
        indicators = this.items.length + this.reduceIndicators
      }
      return indicators
    },
    // RETURNERAR TRUE OM MAN ÄR I SLUTET AV LISTAN
    atEndOfList() {
      return this.activeIndicator === (this.items.length + this.reduceIndicators -1)
    },
